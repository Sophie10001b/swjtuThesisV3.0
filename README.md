...
:computer: **更新日志**
- 2026.01.04(swjtuThesis.cls)
  1. 修正了之前模板中图题表题的多行间距与段前段后间距未生效的问题，目前直接使用minipage + vspace近似效果(最低1pt)，同时参照模板要求，可选左右额外缩进4字符

- 2025.11.24(package.tex)
  1. 在引用定义中强制插入了hbox，从而保证在第一行末尾出现引用上标时，后续的标点符号不会被挤压至第二行行首
...
- 2025.04.19(main.tex, swjtuThesis.cls, package.tex)
  ...
  2. 单独设置了图题与表题的前后间距
  3. 去除了上标引用编号与后面文字间的额外空隙
  4. 去除了双页打印中额外空页的页眉页脚，若需要空页的页眉页脚，可以直接注释掉package.tex中的"emptypage"包
...
- 2025.03.18(statement.tex)
  1. 修改了独创性声明中的间距设定，现在使用较大浮动范围的vspace来控制间距，可以更好的保证标题与正文间距的稳定

- 2025.03.17(main.tex)
  1. 更改了natbib宏包中有关参考文献编号对齐，编号与条目间距的设置，目前编码统一为左对齐，同时编号与条目间距与word模板更加接近
  2. 之前版本中参考文献仍然使用了与正文一致的小四字体，最新版本中已经统一为五号字体
     
- 2025.02.14(swjtuThesis.cls)
  1. (BUG fix) 之前版本中由于代码设置问题，调用\songti会将字体设置为黑体，现在已经修复。模板格式影响范围主要集中在页眉和封面部分字体
